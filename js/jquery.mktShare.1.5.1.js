(function($){
	...
	$.mktShare = {
		...
		_appendCafeBlogForm : function(sUrl){
			...
			// windows10 + ie11조합에서 euc-kr로 charset 전환이 불가능하므로 utf-8로 submit하는 모바일쪽으로 임시 대응
			if( this.isMobile() || this._isWindows10AndIE11() ){
		...
		_isIOS9 : function(){
			...
			if( ua.match(/iPad/i) || ua.match(/iPhone/i) ){
				...
				var iosVersionIndex = ua.indexOf("OS "); // ex UA : ...iPhone OS 9_0_2 like...
				var version = ua.substr(iosVersionIndex + 3, 3).replace("_", ".");
				return 9 === parseInt(version, 10)
		...
		_scrapNaverForPC : function(sType,sInstanceUrl){
			...
			if(this._isIE11()){
				...
				var popupDoc = this._elCafeBlogOpenWindow.document;
				
				try{
					var popupCharset = popupDoc.charset;
					popupDoc.charset = 'euc-kr';
					popupDoc.body.innerHTML = $("#JScrapForm").clone().wrapAll('<div/>').parent().html();
					var elForm = popupDoc.getElementById("JScrapForm");
					elForm.submit();
					popupDoc.charset = popupCharset;
					popupDoc.body.removeChild(elForm);
				}catch(e){
		...
		// windows10 + IE11 카페퍼가기 임시대응 함수
		_scrapNaverForWindows10AndIe11 : function(sType,sInstanceUrl){
		...
		band : function(sInstanceUrl){
			...
			if(this.isMobile()){
				...
				//_sendApp : function(sMessage,sIosUrl,sAndroidPakace,sIframeId)
				this._sendApp(sBandUrl,"https://itunes.apple.com/app/id" + "542613198" + "?mt=8","com.nhn.android.band","_shareBandIframe");

			}else{

				var nTop = ($(window).height() - 533) / 2;
				var nLeft = ($(window).width() - 418) / 2;
				window.open("http://band.us/plugin/share?body="+sEncodedMessage, "SHARE_BAND", "width=418, height=533, resizable=no"+",top="+ nTop +",left=" + nLeft);
		...
		kakaoStory : function(sInstanceUrl){
			...
			if(this.isMobile()){
				...
				//_sendApp : function(sMessage,sIosUrl,sAndroidPakace,sIframeId)
				this._sendApp(sKakaoStoryUrl,"https://itunes.apple.com/app/id" + "486244601" + "?mt=8","com.kakao.story","_shareKakaoStoryIframe");
		...
		naverCafe : function(sInstanceUrl){
			...
			if(this.isMobile()){
				...
				if( this._isWindows10AndIE11() ){
					this._scrapNaverForWindows10AndIe11("cafe",sInstanceUrl);
				}else{
		...
		naverBlog : function(sInstanceUrl){
			...
			if(this.isMobile()){
				...
				if( this._isWindows10AndIE11() ){
					...
					/*
					http://section.blog.naver.com/share/guide.nhn#callout_dialog_mobile
		...
		pholar : function(sPholarName,sInstanceUrl){
			//http://www.pholar.co/spi/rephol?service=npay&url=http%3A%2F%2Fcampaign.naver.com%2F7000p
			...
			var sServiceName = "";
			if(typeof sPholarName === "string"){
				sServiceName = sPholarName;
			}else{
				sServiceName = this._getMessage("pholarName");
			}

			var sEncodedServiceName = encodeURIComponent(sServiceName);
			...
			if(this.isMobile()){
				...
				var nTop = ($(window).height() - 720) / 2;
				var nLeft = ($(window).width() - 504) / 2;

				var sPcFacebookUrl = "http://www.pholar.co/spi/rephol?service="+sEncodedServiceName+"&url=" + sEncodedUrl;
				window.open(sPcFacebookUrl, "SHARE_PHOLAR","width=630, height=790"+",top="+ nTop +",left=" + nLeft);
